...
#include "config/core.h" // ANTI_MAYAP_CHEAT, DEFTYPE_MAX, DEFTYPE_MIN, DEVOTION_REFLECT_DAMAGE,
#include "status.h"
...
#define set_sc_with_vfx(skill, sc, icon, flag) do { set_sc((skill), (sc), (icon), (flag)); if((icon) < SI_MAX) status->dbs->RelevantBLTypes[(icon)] |= BL_SCEFFECT; } while(0)
	...
	set_sc( AL_INCAGI            , SC_INC_AGI         , SI_INC_AGI         , SCB_SPEED );
	set_sc( AL_DECAGI            , SC_DEC_AGI         , SI_DEC_AGI         , SCB_SPEED );
	...
	set_sc( PR_IMPOSITIO         , SC_IMPOSITIO       , SI_IMPOSITIO       , SCB_NONE );
	...
	set_sc( MC_LOUD              , SC_SHOUT        , SI_SHOUT           , SCB_NONE );
	...
	set_sc( SN_SIGHT             , SC_TRUESIGHT       , SI_TRUESIGHT       , SCB_INT | SCB_DEX | SCB_LUK | SCB_CRI | SCB_HIT );
	...
	set_sc( TK_RUN               , SC_STRUP           , SI_STRUP           , SCB_NONE );
	...
	set_sc( NJ_NEN               , SC_NJ_NEN             , SI_NJ_NEN             , SCB_INT );
	...
	set_sc( CASH_BLESSING        , SC_BLESSING        , SI_BLESSING        , SCB_NONE );
	set_sc( CASH_INCAGI          , SC_INC_AGI         , SI_INC_AGI         , SCB_SPEED );
	...
	set_sc( HLIF_CHANGE          , SC_HLIF_CHANGE          , SI_BLANK           , SCB_INT );
	...
	set_sc( MER_BLESSING         , SC_BLESSING        , SI_BLESSING        , SCB_NONE );
	set_sc( MER_INCAGI           , SC_INC_AGI         , SI_INC_AGI         , SCB_SPEED );
	...
	set_sc( GD_BATTLEORDER       , SC_GDSKILL_BATTLEORDER    , SI_BLANK           , SCB_INT | SCB_DEX );
	...
	**/
	...
	set_sc( RK_GIANTGROWTH       , SC_GIANTGROWTH       , SI_GIANTGROWTH       , SCB_NONE );
	...
	* Arch Bishop
	**/
	set_sc( AB_ADORAMUS          , SC_ADORAMUS        , SI_ADORAMUS        , SCB_SPEED );
	...
	**/
	...
	set_sc( LG_INSPIRATION       , SC_INSPIRATION     , SI_INSPIRATION     , SCB_MAXHP | SCB_WATK | SCB_HIT | SCB_DEX | SCB_INT | SCB_LUK );
	...
	**/
	...
	set_sc( SR_GENTLETOUCH_CHANGE    , SC_GENTLETOUCH_CHANGE          , SI_GENTLETOUCH_CHANGE    , SCB_WATK|SCB_ASPD|SCB_MDEF|SCB_MAXHP );
	...
	**/
	...
	set_sc( KO_KYOUGAKU          , SC_KYOUGAKU       , SI_KYOUGAKU             , SCB_INT | SCB_DEX | SCB_LUK );
	...
	set_sc( ALL_FULL_THROTTLE    , SC_FULL_THROTTLE  , SI_FULL_THROTTLE        , SCB_SPEED | SCB_INT | SCB_DEX | SCB_LUK );
	...
	add_sc( RL_MASS_SPIRAL      , SC_BLOODING );
	...
	// Rebellion
	status->dbs->IconChangeTable[SC_QD_SHOT_READY] = SI_E_QD_SHOT_READY;
	status->dbs->IconChangeTable[SC_HEAT_BARREL_AFTER] = SI_HEAT_BARREL_AFTER;
	...
	status->dbs->ChangeFlagTable[SC_2011RWC] |= SCB_INT | SCB_DEX | SCB_LUK | SCB_BATK | SCB_MATK;
	...
	status->dbs->ChangeFlagTable[SC_STOMACHACHE] |= SCB_INT | SCB_DEX | SCB_LUK | SCB_SPEED;
...
int status_check_skilluse(struct block_list *src, struct block_list *target, uint16 skill_id, int flag) {
	...
	if( sc && sc->count ) {
		...
		if (sc->opt1 >0 && !(sc->opt1 == OPT1_CRYSTALIZE && src->type == BL_MOB) && sc->opt1 != OPT1_BURNING && skill_id != RK_REFRESH && skill_id != SR_GENTLETOUCH_CURE) { //Stunned/Frozen/etc
...
int status_calc_pc_(struct map_session_data* sd, enum e_status_calc_opt opt) {
	...
	if((skill_lv = pc->checkskill(sd,SU_POWEROFLAND))>0)
		bstatus->int_ += 20;
	...
	bstatus->amotion = cap_value(i,((sd->class_&JOBL_THIRD || (sd->class_&MAPID_UPPERMASK) == MAPID_REBELLION) ? battle_config.max_third_aspd : battle_config.max_aspd),2000);
...
void status_calc_bl_main(struct block_list *bl, /*enum scb_flag*/int flag) {
	...
	**/
	...
	if(flag&SCB_HIT) {
		if (st->dex == bst->dex && st->luk == bst->luk)
			st->hit = status->calc_hit(bl, sc, bst->hit, true);
		else
			st->hit = status->calc_hit(bl, sc, bst->hit + (st->dex - bst->dex) + (st->luk/3 - bst->luk/3), true);
	}

	if(flag&SCB_FLEE) {
		if (st->agi == bst->agi && st->luk == bst->luk
			)
			st->flee = status->calc_flee(bl, sc, bst->flee, true);
		else
			st->flee = status->calc_flee(bl, sc, bst->flee +(st->agi - bst->agi) + (st->luk/5 - bst->luk/5), true);
	}
	...
	if(flag&SCB_DEF2) {
		if (st->vit == bst->vit && st->agi == bst->agi)
			st->def2 = status->calc_def2(bl, sc, bst->def2, true);
		else
			st->def2 = status->calc_def2(bl, sc, bst->def2 + (int)( ((float)st->vit/2 - (float)bst->vit/2) + ((float)st->agi/5 - (float)bst->agi/5) ), true);
	}
	...
	if(flag&SCB_MDEF2) {
		if (st->int_ == bst->int_ && st->vit == bst->vit && st->dex == bst->dex)
			st->mdef2 = status->calc_mdef2(bl, sc, bst->mdef2, true);
		else
			st->mdef2 = status->calc_mdef2(bl, sc, bst->mdef2 +(st->int_ - bst->int_) + (int)( ((float)st->dex/5 - (float)bst->dex/5) + ((float)st->vit/5 - (float)bst->vit/5) ), true);
	}
	...
	if(flag&SCB_ASPD) {
		...
		if ( bl->type&BL_HOM ) {
			...
			amotion = status->calc_fix_aspd(bl, sc, amotion);
			st->amotion = cap_value(amotion, ((sd->class_&JOBL_THIRD || (sd->class_&MAPID_UPPERMASK) == MAPID_REBELLION) ? battle_config.max_third_aspd : battle_config.max_aspd), 2000);
...
unsigned short status_calc_watk(struct block_list *bl, struct status_change *sc, int watk, bool viewable)
{
	...
	if (sc->data[SC_SKE])
		watk += watk * 3;
	if (sc->data[SC_HAMI_BLOODLUST])
		watk += watk * sc->data[SC_HAMI_BLOODLUST]->val2 / 100;
	if (sc->data[SC_HLIF_FLEET])
...
signed short status_calc_flee(struct block_list *bl, struct status_change *sc, int flee, bool viewable)
{
	...
	if (sc->data[SC_VOLCANIC_ASH] && status_get_element(bl) == ELE_WATER)	// [Volcanic Ash]: -50% flee on water element
		flee -= flee * 50 / 100;
...
#define SCDEF_LVL_DIFF(bl, src, maxlv, factor) ( ( SCDEF_LVL_CAP((bl), (maxlv)) - SCDEF_LVL_CAP((src), (maxlv)) ) * (factor) )
	...
	case SC_NETHERWORLD:
		...
		break;
	case SC_B_TRAP:
		tick_def2 = (sd ? sd->status.str : bst->str) * 50;
		break;
	default:
...
int status_change_start(struct block_list *src, struct block_list *bl, enum sc_type type, int rate, int val1, int val2, int val3, int val4, int tick, int flag)
{
	...
	switch (type) {
		...
		case SC_GS_MADNESSCANCEL:
			...
			break;
		case SC_HEAT_BARREL:
			if (sc->data[SC_GS_MADNESSCANCEL] || sc->data[SC_P_ALTER])
				return 0;
			break;
		case SC_P_ALTER:
			if (sc->data[SC_GS_MADNESSCANCEL] || sc->data[SC_HEAT_BARREL])
				return 0;
			break;
	}
			...
			case SC_DEVOTION: {
					...
					if ((d_bl = map->id2bl(val1)) && (d_sc = status->get_sc(d_bl)) != NULL && d_sc->count) {
						...
						while (i >= 0) {
							...
							if (d_sc->data[type2]) {
								status->change_start(bl, bl, type2, 10000, d_sc->data[type2]->val1, 0, 0, 0, skill->get_time(status->sc2skill(type2) ,d_sc->data[type2]->val1), (type2 != SC_DEFENDER) ? SCFLAG_NOICON : SCFLAG_NONE);
							}
							i--;
						}
					}
			...
			case SC_JAILED:
				...
				if (sd) {
					if (sd->mapindex != val2) {
						...
						val3 = sd->status.save_point.map; // Use save point.
...
int status_change_end_(struct block_list* bl, enum sc_type type, int tid, const char* file, int line) {
	...
	switch(type) {
		...
		case SC_RAISINGDRAGON:
			...
			break;
		case SC_GRANITIC_ARMOR:	// [Granitic Armor]: -30% HP when the status ends.
			status_zap(bl,st->hp*30/100,0);	
			break;
		case SC_PYROCLASTIC:
...
#define sc_timer_next(t,f,i,d) do { \
	...
	switch(type) {
		...
		case SC_ELECTRICSHOCKER:
			if( --(sce->val4) > 0 ) {
				status->charge(bl, 0, 5 * sce->val1 * st->max_sp / 100 );
				sc_timer_next(1000 + tick, status->change_timer, bl->id, data);
				return 0;
			}
			break;
		...
		case SC_ANGRIFFS_MODUS:
			if(--(sce->val4) >= 0) {
				...
				if ( !status_charge(bl,0,sp) )	// If homunculus' SP reaches 0, skill is canceled automatically.
					break;

				map->freeblock_lock();
				status_damage(NULL,bl,hp,0,0,0);
				dead = !sc->data[type];
				map->freeblock_unlock();
				if ( !dead )
