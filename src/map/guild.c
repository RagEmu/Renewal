	...
	if((g = (struct guild*)idb_get(guild->db,sg->guild_id))==NULL) {
		...
		if ((sd = map->nick2sd(sg->master)) != NULL) {
			...
			if (battle_config.guild_skill_relog_delay)
				guild->block_skill(sd, battle_config.guild_skill_relog_delay);
...
void guild_guildaura_refresh(struct map_session_data *sd, uint16 skill_id, uint16 skill_lv) {
	...
	group = skill->unitsetting(&sd->bl,skill_id,skill_lv,sd->bl.x,sd->bl.y,0, 0);
	if( group ) {
...
int guild_gm_changed(int guild_id, int account_id, int char_id)
{
	...
	if (g->member[0].sd && g->member[0].sd->fd) {
		...
		// Block skill usage to prevent abuse.
		if (battle_config.guild_skill_relog_delay)
			guild->block_skill(g->member[0].sd, battle_config.guild_skill_relog_delay);
	}
