...
int elemental_search_index(enum elemental_type kind, int scale, int class_) {
	...
	if (!class_)
		...
		ARR_FIND(0, ARRAYLENGTH(elemental->db), i, elemental->db[i].db && elemental->db[i].db->vd.class == class_);
...
int elemental_data_received(const struct s_elemental *ele, bool flag)
{
	...
	if( !sd->ed ) {
		...
		status->set_viewdata(&ed->bl, ed->db->vd.class);
...
int read_elemental_skilldb(void) {
	...
	while( fgets(line, sizeof(line), fp) ) {
		...
		if( i == MAX_ELESKILLTREE ) {
			...
			continue;
>>>>>>> c1b4b0b... Remove the platform-dependent variables from struct status_data
		}
