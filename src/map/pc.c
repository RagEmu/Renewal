...
#include "config/core.h" // GP_BOUND_ITEMS, MAX_SPIRITBALL, RENEWAL_DROP, RENEWAL_EXP, SECURE_NPCTIMEOUT
#include "pc.h"
...
* @param sd
* @param interval Duration of SpiritBall
* @param max
* @return 0
*/
...
int pc_skillheal_bonus(struct map_session_data *sd, uint16 skill_id)
{
	...
	if (bonus) {
		switch (skill_id) {
			case AL_HEAL:
				if (!(battle_config.skill_add_heal_rate & 1))
					bonus = 0;
				break;
			case PR_SANCTUARY:
				if (!(battle_config.skill_add_heal_rate & 2))
					bonus = 0;
				break;
			case AM_POTIONPITCHER:
				if (!(battle_config.skill_add_heal_rate & 4))
					bonus = 0;
				break;
			case CR_SLIMPITCHER:
				if (!(battle_config.skill_add_heal_rate & 8))
					bonus = 0;
				break;
			case BA_APPLEIDUN:
				if (!(battle_config.skill_add_heal_rate & 16))
					bonus = 0;
				break;
			case AB_HIGHNESSHEAL:
				if (!(battle_config.skill_add_heal_rate & 32))
					bonus = 0;
				break;
		}
	}
...
void pc_read_skill_tree(void)
{
	...
	safesnprintf(config_filename, sizeof(config_filename), "%s/""skill_tree.conf", map->db_path);
...
int pc_readdb(void) {
	...
	sprintf(line, "%s/""attr_fix.txt", map->db_path);
	...
	sprintf(line, "%s/""statpoint.txt", map->db_path);
	fp=fopen(line,"r");
	if(fp == NULL){
