...
void vending_vendinglistreq(struct map_session_data* sd, unsigned int id)
{
	...
	if (!pc_can_give_items(sd) || !pc_can_give_items(vsd)) { // Check if both GMs are allowed to trade
		clif->message(sd->fd, msg_sd(sd, 246)); // GM is not allowed to trade
		return;
	}
...
void vending_purchasereq(struct map_session_data* sd, int aid, unsigned int uid, const uint8* data, int count)
{
	...
	for (i = 0; i < count; i++) {
		...
		if (vend[j].amount < amount) { // Send more quantity is not a hack (an other player can have buy items just before)
...
void vending_openvending(struct map_session_data* sd, const char* message, const uint8* data, int count)
{
	...
	if (map->save_settings & 2) // Avoid invalid data from saving
		chrif->save(sd, 0);
...
bool vending_searchall(struct map_session_data* sd, const struct s_search_store_search* s)
{
	...
	for (idx = 0; idx < s->item_count; idx++) {
		...
		if (!searchstore->result(s->search_sd, sd->vender_id, sd->status.account_id, sd->message, it->nameid, sd->vending[i].amount, sd->vending[i].value, it->card, it->refine)) { // Result set full
			return false;
		}
	}
	return true;
