		...
		if (sc && sc->count) {
			if (sc->data[SC_DANCING])
				...
				if (sc->data[SC_PROPERTYWALK]
				 ...
				 && skill->unitsetting(bl,sc->data[SC_PROPERTYWALK]->val1,sc->data[SC_PROPERTYWALK]->val2,x0, y0,0, 0)
				) {
...
bool inter_config_read_database_names(const char *filename, const struct config_t *config, bool imported)
{
	...
	if ((setting = libconfig->lookup(config, "inter_configuration/database_names/trade")) == NULL) {
		if (imported)
			return true;
		ShowError("inter_config_read: inter_configuration/database_names/trade was not found in %s!\n", filename);
		return false;
	}
	...
	libconfig->setting_lookup_mutable_string(setting, "buyingstore_merchants_db", map->buyingstore_merchants_db, sizeof(map->buyingstore_merchants_db));
	libconfig->setting_lookup_mutable_string(setting, "buyingstore_data_db", map->buyingstore_data_db, sizeof(map->buyingstore_data_db));
