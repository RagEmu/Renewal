...
int char_make_new_char_sql(struct char_session_data* sd, const char* name_, int str, int agi, int vit, int int_, int dex, int luk, int slot, int hair_color, int hair_style, int race, uint8 sex)
{
	...
	char esc_name[NAME_LENGTH*2+1];
	int char_id, flag, i;
	int items_length = VECTOR_LENGTH(start_items);
...
void char_parse_char_create_new_char(int fd, struct char_session_data* sd)
{
	...
	if (!enable_char_creation) {
		...
		result = chr->make_new_char_sql(sd, RFIFOP(fd,2), 1, 1, 1, 1, 1, 1, RFIFOB(fd,26),RFIFOW(fd,27),RFIFOW(fd,29), RACE_HUMAN, 'U');
#else
		result = chr->make_new_char_sql(sd, RFIFOP(fd,2), RFIFOB(fd,26), RFIFOB(fd,27), RFIFOB(fd,28), RFIFOB(fd,29), RFIFOB(fd,30), RFIFOB(fd,31), RFIFOB(fd,32), RFIFOW(fd,33), RFIFOW(fd,35), RACE_HUMAN, 'U');
#endif
	}
...
/**
 * Defines start_items_doram based on '(...)/player/new/start_items_doram'.
 *
 * @param setting The already retrieved start_item setting.
 */
void char_config_set_start_item_doram(const struct config_setting_t *setting)
{
	int i, count;
	...
	VECTOR_ENSURE(start_items_doram, count, 1);
	...
	for (i = 0; i < count; i++) {
		...
		if (libconfig->setting_lookup_int(t, "loc", &start_item.loc) != CONFIG_TRUE)
			start_item.loc = 0;
		VECTOR_PUSH(start_items_doram, start_item);
	}
}
...
bool char_config_read_player_new(const char *filename, const struct config_t *config, bool imported)
{
	...
	if ((setting2 = libconfig->setting_get_member(setting, "start_items_doram")))
		chr->config_set_start_item_doram(setting2);

	// start_point / start_point_pre
	if ((setting2 = libconfig->setting_get_member(setting, "start_point"))) {
		const char *str = NULL;
...
void char_defaults(void)
{
	...
	chr->config_set_start_item_doram = char_config_set_start_item_doram;
