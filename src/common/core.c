 ...
 */
//#define I_AM_AWARE_OF_THE_RISK_AND_STILL_WANT_TO_RUN_RAGEMU_AS_ROOT
...
bool usercheck(void)
{
	...
	if (sysinfo->is_superuser()) {
		if (!isatty(fileno(stdin))) {
			...
			return true;
#else  // BUILDBOT
			ShowFatalError("You are running RagEmu with root privileges, it is not necessary, nor recommended. "
					"Aborting.\n");
			return false; // Don't allow noninteractive execution regardless.
#endif  // BUILDBOT
		}
		ShowError("You are running RagEmu with root privileges, it is not necessary, nor recommended.\n");
#ifdef I_AM_AWARE_OF_THE_RISK_AND_STILL_WANT_TO_RUN_RAGEMU_AS_ROOT
#ifndef BUILDBOT
#warning This RagEmu build is not eligible to obtain support by the developers.
#warning The setting I_AM_AWARE_OF_THE_RISK_AND_STILL_WANT_TO_RUN_RAGEMU_AS_ROOT is deprecated and should not be used.
#endif  // BUILDBOT
#else // not I_AM_AWARE_OF_THE_RISK_AND_STILL_WANT_TO_RUN_RAGEMU_AS_ROOT
		...
		ShowNotice("Resuming operations with root privileges. "
				...
				"and the RagEmu developers won't be able to help you."
				CL_RESET "\n");
#endif // I_AM_AWARE_OF_THE_RISK_AND_STILL_WANT_TO_RUN_RAGEMU_AS_ROOT
	}
#endif // not _WIN32
	return true;
}
